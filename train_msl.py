...
def train_and_eval_on_MSL(model_type, model, num_epochs, learning_rate, window_size, cond_window_size, batch_size, early_stop_patience=100, use_validation=False):
    ...
    if model.prob_decoder:
        save_folder = 'saved_models/MSL/' + model_type + model.flow_type + '_prob_decoder/' if model.flow_type != None else 'saved_models/MSL/' + model_type + '_prob_decoder/'
        if not os.path.exists(save_folder):
            os.makedirs(save_folder)
        torch.save(model, save_folder + model_type + '-' + machine_name + '.pth')
    else:
        save_folder = 'saved_models/MSL/' + model_type + model.flow_type + '/' if model.flow_type != None else 'saved_models/MSL/' + model_type + '/'
        if not os.path.exists(save_folder):
            os.makedirs(save_folder)
...
def main():
    ...
    model_type = 'tcn_vae' #'cnn_vae', 'cnn_cvae', 'tcn_vae', 'tcn_cvae'
    conditional = False
    ...
    if len(channels) != num_levels*convs_per_level+1:
        print('channels wrong size! -- channels must be of length {}'.format(num_levels*convs_per_level+1))
        return
